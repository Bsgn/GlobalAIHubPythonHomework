...
class Lessons():
    ...
    def gradeInfo(self):
        #calculating average
        result = (self.kwargs["midterm"]*0.3) + (self.kwargs["final"]*0.5) + (self.kwargs["project"]*0.2)
    ...
    if (log_name == name) and (log_surname == surname):
        ...
        while les_entry:
            ...
            if len(lesson_list) < 5:
                cont_entry = input("Do you want to enter another lesson? (press 'y' if you want to enter another lesson) : ").lower()
                if cont_entry == "y":
                    les_entry = True
                else:
                    cont_entry = input ("Are you sure to entered all of your lessons? (press 'y' if you are sure) : ").lower()
                    if cont_entry == "y":
                        les_entry = False
                    else:
                        les_entry = True
...
while calc_grade:
    ...
    while sl_les:
        print("\n")
        selected_les = input("Select a lesson to calculating the grade : ").title()
